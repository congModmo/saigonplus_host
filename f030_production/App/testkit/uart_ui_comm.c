...
void cmd_import_key_handle(){
	...
	if(!crc32_verify(des+4, strlen(des+4), *((uint32_t *)des))){
		testkit_direct_report(UART_UI_CMD_IMPORT_KEY, 0);
		return;
	}
	des=fotaCoreBuff+2048;
	if(!crc32_verify(des+4, strlen(des+4), *((uint32_t *)des))){
...
//	testkit_direct_report(UART_UI_CMD_IMPORT_KEY, 1);
...
static void uart_ui_command_handle(uint8_t *frame, size_t size)
{
	...
	case UART_UI_CMD_FACTORY_RESET:
		uart_ui_command_send(UART_UI_CMD_FACTORY_RESET, UART_UI_RES_OK);
		//factory_reset();
		break;
	case UART_UI_CMD_PING:
		uart_ui_command_send(UART_UI_RES_PING, UART_UI_RES_OK);
		break;
...
void uart_ui_init()
{
	bsp_uart_ui_init(&uartUiRingbuf);
	slip_init(&slip, slip_buff, 512, bsp_uart_ui_send_byte);
}
