...
void transport_send_cmd(uint8_t type, uint8_t param){
	transport.serial.send(&type, 1, SLIP_FRAME_BEGIN);
	transport.serial.send(&param, 1, SLIP_FRAME_END);
}

static void transport_send_file_request(char *fileName){
	uint8_t cmd=DEVICE_REQUEST_FILE_INFO;
	transport.serial.send(&cmd, 1, SLIP_FRAME_BEGIN);
	transport.serial.send((uint8_t *)fileName, strlen(fileName), SLIP_FRAME_END);
}
